...
- **桌面/Android**：模板 `DevAssetSource` 直读磁盘，CLI 在变化时广播 `asset_changed`，删除时广播显式 `asset_removed`；app 侧 `pump_live_assets` 用 `cx.remove_asset::<ImgResourceLoader>` 驱逐缓存键并 `refresh_windows`。Android 的字节由 CLI 先经 adb 写入 app files dir，删除则经 `run-as` 移除对应私有文件，再通知 runtime（app 沙盒读不到项目目录）。入口必须**先装 asset source 再 init_live**，否则 hello 能力位有顺序竞态。
...
- **通道可靠性加固（2026-09-20）**：超帧限的资产在编码前拒绝并显式告警（iOS 上无法推送也无法热重载）；只对**实际投递成功**的文件广播 `asset_changed`——广播一次失败的推送会让 app 驱逐一个它无法回填的缓存条目；hello 事件改为**先注册连接再上报**，重连触发的 manifest/hash 对账和带 `transfer_id` 的定向补发命中刚连上的 app；资源变更现在按 `assets_begin` → 内容/删除消息 → `assets_commit` 成批交付，runtime 在 commit 前只暂存 UI 事件；iOS bytes 在写入前校验声明 SHA-256，并通过同目录临时文件 rename 提交；应用可声明必需资源，桌面/Android AssetSource 成功读取并校验后另报 `assets_required_loaded`；runtime 另以 `assets_received` 报告传输到达，缓存失效仍需 `assets_applied`；写入失败拆除整条连接（否则读半会永远等帧）并由 `write_failed` 标志上抛到 live 循环。
