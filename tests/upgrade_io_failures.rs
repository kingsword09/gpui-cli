//! Verify recovery after deterministic storage and permission failures.
...
fn run(root: &Path, args: &[&str], io_failure: Option<&str>) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_gpui"));
    command.current_dir(root).args(args);
    if let Some(failure) = io_failure {
        command.env("GPUI_UPGRADE_IO_FAILURE", failure);
    } else {
        command.env_remove("GPUI_UPGRADE_IO_FAILURE");
    }
...
fn snapshot(root: &Path) -> Vec<(&'static str, Option<Vec<u8>>)> {
    ...
    .into_iter()
    .map(|relative| (relative, fs::read(root.join(relative)).ok()))
    .collect()
}

#[test]
fn storage_and_permission_failures_roll_back_real_multifile_apply() {
    let dir = tempfile::tempdir().unwrap();
    for (index, (failure, marker)) in [
        ("backup:permission_denied", "injected_permission_denied"),
        ("project:storage_full", "injected_storage_full"),
    ]
    .into_iter()
    .enumerate()
    {
        let project = dir.path().join(format!("io-probe-{index}"));
        let project_string = project.to_str().unwrap();
        let init = run(
            dir.path(),
            &[
                "init",
                &format!("io-probe-{index}"),
                "--path",
                project_string,
                "--targets",
                "macos",
        ...
        assert!(
            ...
            "failure={failure}, stderr={stderr}"
        );
        assert!(
            stderr.contains("recovery_state=RolledBack"),
            "failure={failure}, stderr={stderr}"
        );
        assert_eq!(snapshot(&project), before, "failure={failure}");
        ...
        let transactions = fs::read_dir(project.join(".gpui/upgrade/transactions"))
            ...
            .unwrap();
        assert_eq!(transactions.len(), 1, "failure={failure}");
